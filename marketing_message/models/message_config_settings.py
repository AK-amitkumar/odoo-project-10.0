...
class Audiosettings(models.Model):
    ...
    def get_default_info(self, fields):
        ...
        return {
            'APP_ID': Param.get_param('APP_ID', default=''),
            'API_KEY': Param.get_param('API_KEY', default=''),
            'SECRET_KEY': Param.get_param('SECRET_KEY', default=''),

            'app_key': Param.get_param('app_key', default=''),  # 嘟嘟应用账号标识
            'ext_orgCode': Param.get_param('ext_orgCode', default=''),  # 客户组织编码
            'cust_account': Param.get_param('cust_account', default=''),  # 计费账号标识
            'org_tempKey': Param.get_param('org_tempKey', default=''),  # 计费账号临时密钥
            'ext_terminalCode': Param.get_param('ext_terminalCode', default=''),  # 使用通讯能力的用户ID
        }
